...
如性能监控、自动重启、负载均衡等，使用简单。
    ...
    Node运行在Chrome的JavaScript运行时平台上，把该平台优雅称为V8引擎
都是以单线程的方式运行的。

    通过一个主进程（Master）管理多个子进程（Worker）方式实现集群功能；

    主进程和子进程之间通过IPC（进程间通信）实现进程间的通信，进程之间通过send方法
    发送信息，通过监听message事件收取信息，这和cluster模块继承的EventEmitter对象一致；

    Node提供了Cluster模块，可以生成多个工作线程来共享同一个TCP连接
